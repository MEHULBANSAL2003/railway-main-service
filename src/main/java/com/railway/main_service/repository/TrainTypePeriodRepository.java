...
@Repository
public interface TrainTypePeriodRepository extends JpaRepository<TrainTypePeriodEntity, Long> {

  // Is the train type active on a given date? (any period covers that date)
  @Query("""
    SELECT COUNT(p) > 0 FROM TrainTypePeriodEntity p
    WHERE p.trainType.typeId = :typeId
  ...
  @Query("""
    SELECT p FROM TrainTypePeriodEntity p
    WHERE p.trainType.typeId = :typeId
      AND p.effectiveFrom <= :date
      AND (p.effectiveTill IS NULL OR p.effectiveTill >= :date)
    ORDER BY p.effectiveFrom DESC
    """)
  Optional<TrainTypePeriodEntity> findActivePeriod(@Param("typeId") Long typeId, @Param("date") LocalDate date);
  ...
  @Query("""
    ...
    ORDER BY p.effectiveFrom DESC
    """)
  Optional<TrainTypePeriodEntity> findOpenPeriod(@Param("typeId") Long typeId);

  // All periods for a train type — full history ordered by date
  @Query("""
    SELECT p FROM TrainTypePeriodEntity p
    WHERE p.trainType.typeId = :typeId
    ORDER BY p.effectiveFrom ASC
    """)
  List<TrainTypePeriodEntity> findAllByTypeId(@Param("typeId") Long typeId);
  ...
  @Query("""
    ...
    ORDER BY p.effectiveFrom ASC
    """)
  List<TrainTypePeriodEntity> findFuturePeriods(@Param("typeId") Long typeId, @Param("date") LocalDate date);

  // Close current open period by setting effectiveTill
  @Modifying
  @Query("""
    UPDATE TrainTypePeriodEntity p
  ...
  @Modifying
  @Query("""
    DELETE FROM TrainTypePeriodEntity p
    WHERE p.trainType.typeId = :typeId
