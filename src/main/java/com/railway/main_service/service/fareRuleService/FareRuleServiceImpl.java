...
public class FareRuleServiceImpl implements FareRuleService {
  ...
  public FareRuleResponse addFareRule(AddFareRuleRequest request) {
    ...
    if (fareRuleRepository.existsByTrainType_TypeCodeAndCoachType_TypeCodeAndEffectiveFrom(
      trainCode, coachCode, request.getEffectiveFrom())) {
    ...
    fareRuleRepository.findOpenRule(trainCode, coachCode).ifPresent(existing -> {
  ...
  @Override
  public List<FareRuleResponse> getAllForAdmin(String trainTypeCode, String coachTypeCode) {
    ...
    return fareRuleRepository.findAllForAdmin(tc, cc)
